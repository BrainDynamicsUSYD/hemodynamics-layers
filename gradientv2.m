...
if n > 2
   ...
   g(2,:) = (f(3,:) - f(1,:))/(h(2)-h(1))/2;
   g(n,:) = (3/2*f(n,:) - 2*f(n-1,:) +1/2*f(n-2,:))/(h(end)-h(end-1));
   g(n-1,:) = (f(n,:) - f(n-2,:))/(h(end)-h(end-1))/2;
end

% Take centered differences on interior points
if n > 4
   h = h(5:n) - h(1:n-4);
   g(3:n-2,:) = bsxfun(@rdivide,(-1/3*f(5:n,:)+8/3*f(4:n-1,:)-8/3*f(2:n-3,:)+1/3*f(1:n-4,:)),h);
end
...
for k = 2:ndim
   ...
   if n > 2
      ...
      g(:,2,:) = (nf(:,3,:) - nf(:,1,:))/(h(2)-h(1))/2;
      g(:,n-1,:) = (nf(:,n,:) - nf(:,n-2,:))/(h(end)-h(end-1))/2;
   end

   % Take centered differences on interior points
   if n > 4
      h = h(5:n) - h(1:n-4);
      g(:,3:n-2,:) = bsxfun(@rdivide,(-1/3*nf(:,5:n,:)+8/3*nf(:,4:n-1,:)-8/3*nf(:,2:n-3,:)+1/3*nf(:,1:n-4,:)),h);
   end
